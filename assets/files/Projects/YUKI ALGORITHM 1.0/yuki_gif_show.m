...
% % MeanBest=rand(Dim,1)*(Global_bnd(2)-Global_bnd(1))+ Global_bnd(1);
% % Center=rand(Dim,1)*(Global_bnd(2)-Global_bnd(1))+ Global_bnd(1);
...
Dist_MeanBest=Center-MeanBest; %% calcualte the distance between the first and second only best solution  
...
if  It_BestFit<Optimum_Fit 
	...
	Center=It_BestPos;  % replace the first reference and set the old reference a second reference
end
...
%% OPTIONAL: desplay the progress every 100 Its
    if mod (It,100)==1 % ever 100 show the actual best fitness (not absolute best)
        disp(It_BestFit);
    end
